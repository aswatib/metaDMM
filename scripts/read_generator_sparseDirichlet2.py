...
#    alpha = mean_proportions * (((mean_proportions * (1 - mean_proportions)) / variance_proportions) - 1) # Moment as per Wikipedia
...
def calculate_alpha(data):
    ...
    mean_proportions = np.mean(data / n[:, None], axis=0)
    variance_proportions = np.var(data / n[:, None], axis=0, ddof=1)    
    # Add a small constant to variance_proportions to avoid division by zero
    variance_proportions += 1e-9
    alpha = mean_proportions * (((mean_proportions * (1 - mean_proportions)) / variance_proportions) - 1)
    # Set negative alpha values to a small positive constant
    alpha[alpha < 0] = 1e-6
    alpha_0 = np.sum(alpha)
    return alpha, alpha_0
...
num_samples = 20
sample_folder = "sparseDirichlet2"
stats_file = "stats_sparseDirichlet2.csv"
sparsity_factor = 0.01  # Change this value to test different alphas
